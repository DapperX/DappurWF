	...
	Parser = (function() {
		...
		Parser.prototype.parseInline = function(text, whiteList, clearHolders, enableAutoLink) {
			...
			text = text.replace(/(^|[^\\])(`+)\$(.+?)\$\2/mg, (function(_this) {
				return function() {
					...
					return matches[1] + _this.makeHolder('<code class="tex">' + katex.renderToString(matches[3],{displayMode: false,throwOnError:false}) + '</code>');
				};
			})(this));
			text = text.replace(/(^|[^\\])(`+)(.+?)\2/mg, (function(_this) {
				return function() {
		...
		Parser.prototype.parseCode = function(lines, parts) {
			...
			if (str.match(/^\s*$/)) {
				...
				if(lang=="$"){
					return "<div class='tex'" + (!!rel ? " rel=\"" + rel + "\"" : '') + ' >' + katex.renderToString(str,{displayMode: true,throwOnError:false}) + "</div>";
				}
